...
<script type="text/javascript">
    document.addEventListener("DOMContentLoaded", function() {
        let posyandu = $('[name="posyandu"]').parent().parent()

        setTimeout(function() {
            if ('Kader' !== getSelectedRole()) posyandu.hide()
        }, 100)

        $('[name="role"]').change(function() {
            if ('Kader' === getSelectedRole()) posyandu.show()
            else posyandu.hide()
        })

        function getSelectedRole() {
            return $('[name="role"]').find('option:selected').text()
        }
    });
</script>